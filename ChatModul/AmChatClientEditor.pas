  ...
  const
   ...
   type
    ...
    //  ListThread:TAmClientListPot;
      ...
      Procedure  Start( aAddressSocket:string;  aPortSokect:integer;  aLog:TamLogString; MiliSecondsTimeOutSocket:Cardinal=INFINITE);
...
var
 S: TFileStream;
     ...
     begin
       if not AmControlCheckWork(Message.MsgFiles) then exit;
       Message.MsgFiles.PhotoDefaultFileName:='E:\Red 10.3\Projects\socketClientServer\Resources\Files\Без имени-1.png';
     end;
...
begin
  ...
  begin
      ...
      if R then
      begin
        ContactObj:=AmJson.LoadText(FRM.ContactObj);
        try
...
begin
    ...
    begin
        ...
        begin
          if ActivDialog.Check(FRM.GroopId,ConstAmChat.TypeUser.Groop) then
          begin
              Arr:= AmJson.LoadArray(FRM.JsonArray);
              try
...
begin
    ...
    begin
       ...
       begin
         ContactObj:= amJson.LoadText(FRM.ObjContact);
         try
...
begin
   inherited create;
    ...
    ObjUser:= amJson.LoadFile(FDir+FnObjUser);
    ...
    SocketFile:=TAmClientSockFile.Create(LogMain);
end;
...
Procedure TChatClientEditor.Start( aAddressSocket:string;  aPortSokect:integer;  aLog:TamLogString; MiliSecondsTimeOutSocket:Cardinal=INFINITE);
begin
        Stop;
...
begin
     List:=amJson.LoadText(ContactsObjJsonString);
     try
...
var  R:TAmResult16;
    Procedure Local_UpDateContact(JsonStr:string);
     ...
     begin
       ...
       ContactObj:= amJson.LoadText(JsonStr);
       try
             HisUserId:= ContactObj['Id'].Value;
             HisTypeUser:=ContactObj['TypeUser'].Value;
             if (AmInt(HisUserId,0)=0)
...
var s:string;
   ...
   begin
        ObjNewMsg:=amJson.LoadText(ObjMessageJsonString);
        try
...
begin
     ...
     ObjMsgContact:= amJson.LoadText(ObjContactJsonString);
...
begin
   CloseDialog;
   FMessage:= amJson.LoadText(ObjMessageJsonString);
   FUsers:= amJson.LoadText(ObjUsersJsonString);
...
begin
  try
   N:= amJson.LoadText(ObjOneMessageJsonString);
   try
...
begin
  try
   ...
   Ns:= amJson.LoadText(ObjMessageJsonString);
   try
...
Names:string;
begin
   N:= amJson.LoadText(ObjUserOneJsonString);
   try

...
uses
 ...
 type
  ...
  TAmClientMessageFilesZipControl =class(TEsLayout)
    ...
    strict private
     ...
     private
      ...
      // создает новую картинку и заносит ее в ListControl  Stream получаю когда читаю zip архив ZipFileName в SetZipFile
        procedure LoadNewImg(Fn:String);
    ...
    //   FListPhoto:TamListVar<String>;
     ...
     // Property СollagePhoto:Es.Images.TEsImage read FImg;
      ...
      Property PhotoDefaultFileName :string read FPhotoDefaultFileName Write SetPhotoDefaultFileName;
  ...
  // лист бокс используется когда на форму перетаскиваются файлы что бы их отобразить (юзер пытается отправить файлы готовит их)
   type
    TAmClientListBoxFilesAtSend = class (TAmClientListBoxFilesCustom)
      private
 ...
 { type
   ...
   type
    TAmClientListBoxFilesAtView = class (TAmClientListBoxFilesCustom)
      private
       Procedure  PopapClickToItem(S:Tobject;NameItem:String);
      protected
      public
   ...
   type
    TAmClientListBoxFilesAtViewZip = class (TAmClientListBoxFilesCustom)
...
s:string;
begin
    Json:= AmJson.LoadStream(JsonStream);
    try
     s:= Json.ToJSON();
...
begin
    JsonArray:= AmJson.LoadArray(ArrJsonString);
    try
...
procedure TAmClientMessageFilesZipControl.LoadNewImg(Fn:String);
